...
export const deposit = async (name: string, amount: number, userId: string) => {
...
export const transfer = async (
  ...
  try {
    ...
    senderAcc.history.push(
      `Transfer to ${receiver.toLowerCase()}: ${amount} - ${time}`,
    );
...
export const getBalance = async (name: string, userId: string) => {
  try {
...
export const getHistory = async (name: string, userId: string) => {
  try {
    const account = await Account.findOne({ name, userId });
    if (!account) throw new Error("Account not found");
